...
async fn main() -> Result<()> {
    ...
    let cors = warp::cors()
        ...
        .allow_origins(vec!["http://localhost:3000", "https://example.com"])
    ...
    let health_checker = warp::path!("api" / "healthchecker")
        .and(warp::get())
        .and_then(handler::health_checker_handler); // Corrected function name here
    ...
    let note_routes_id = note_router_id
        ...
        .and(warp::body::json())
        .and(with_db(db.clone()))
        .and_then(handler::edit_note_handle)
        .or(note_router_id
            .and(warp::get())
            .and(with_db(db.clone()))
