...
pub async fn notes_list_handler(opts: FilterOptions, db: Database) -> WebResult<impl Reply> {
...
pub async fn create_note_handler(body: CreateNoteSchema, db: Database) -> WebResult<impl Reply> {
    let note = db.create_note(&body).await.map_err(|e| reject::custom(e))?;

    Ok(with_status(json(&note), StatusCode::CREATED))
}

pub async fn get_note_handler(id: String, db: Database) -> WebResult<impl Reply> {
...
pub async fn delete_note_handler(id: String, db: Database) -> WebResult<impl Reply> {
